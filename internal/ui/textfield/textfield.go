...
func (t *TextField) Focus() { t.field.Focus() }
...
func (t *TextField) Draw(screen *ebiten.Image, x, y float64) {
	...
	// Draw border
	vector.StrokeRect(screen, float32(t.bounds.Min.X), float32(t.bounds.Min.Y), float32(width), float32(height), 1, color.White, false)

	// Draw background
	vector.FillRect(screen, float32(t.bounds.Min.X), float32(t.bounds.Min.Y), float32(width), float32(height), color.RGBA{0, 0, 0, 255}, false)
