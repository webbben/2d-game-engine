...
func (w *World) TimeLapse(newTime clock.GameTime) {
	if w.AwaitingTimeLapse.Load() {
		logz.Panic("AwaitingTimeLapse was already set; either the last time lapse didn't clear this, or we accidentally called a time lapse before the last one could trigger")
	}
	if w.TimeLapseTo.Load() != nil {
		logz.Panic("TimeLapseTo wasn't nil; either the last time lapse didn't clear this, or we accidentally called a time lapse before the last one could be triggered")
	}
	...
	w.AwaitingTimeLapse.Store(true)
	w.TimeLapseTo.Store(&newTime)
